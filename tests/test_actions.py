...
def test_do_invite_friend(app, users, friends):
    ...
    do_invite_friend(
        strategy, from_user=from_user, to_user_email=to_user_email, message=message
    )
    ...
    do_invite_friend(
        strategy, from_user=from_user, to_user_email=to_user.email, message=message
    )
    assert mock_request_email.call_count == 1
    assert (
        strategy.storage.friendshipRequest.objects.filter(
...
def test_new_user_created(strategy, users, user_cls):
    ...
    do_invite_friend(strategy, from_user=users[0], to_user_email=email, message="")
    do_invite_friend(strategy, from_user=users[1], to_user_email=email, message="")
    user = user_cls.objects.create(email=email, username="newuser", password="pw")
...
def test_load_friendship_request(strategy, users):
    from_user = users[0]
    to_user = users[1]
    token, request = do_invite_return_token_request(strategy, from_user, to_user)
...
def test_accept_friendship_request(strategy, users):
    ...
    assert (
        strategy.storage.friends.objects.filter(user1=from_user, user2=to_user).count()
        == 1
    )
...
def test_reject_friendship_request(strategy, users):
    ...
    assert (
        strategy.storage.friendshipRequest.objects.get(pk=request.id).rejected_at
        is not None
    )
...
def test_friendship_request_list(users, strategy):
    ...
    do_invite_friend(strategy, from_user=user_a, to_user_email=user_b.email, message="")
    res = friendship_request_list(strategy, user_a)
    assert len(res) == 3
